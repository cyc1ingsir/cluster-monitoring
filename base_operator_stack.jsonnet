...

{
  ...
  if vars.enablePersistence.grafana then {
    ...
    storage:
      local pvc = k.core.v1.persistentVolumeClaim;
      pvc.new() +
      pvc.mixin.metadata.withNamespace($.values.common.namespace) +
  ...
  ingress+:: {
    ...
    //     ingress.mixin.metadata.withNamespace($.values.common.namespace) +
    ...
    //     secret.mixin.metadata.withNamespace($.values.common.namespace),
  } + if vars.UseProvidedCerts then {
    secret:
      utils.newTLSSecret('ingress-secret', $.values.common.namespace, vars.TLSCertificate, vars.TLSKey),
  } else {},
}
